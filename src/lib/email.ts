...
export async function sendTeamOrderNotification(order: OrderPayload) {
  ...
  await resend.emails.send({
    ...
    html: `
      ...
      <p><strong>Customer:</strong> ${order.customer.firstName} ${order.customer.lastName} — ${order.customer.email} — ${order.customer.phone}</p>
      <p><strong>Address:</strong> ${order.customer.street}, ${order.customer.city} ${order.customer.postal}</p>
...
export async function sendCustomerOrderConfirmation(order: OrderPayload) {
  ...
  await resend.emails.send({
    ...
    html: `
      <h2>Thanks, ${order.customer.firstName} — your order is confirmed</h2>
      ...
      <p>Delivering to: ${order.customer.street}, ${order.customer.city} ${order.customer.postal}</p>
